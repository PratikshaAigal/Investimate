...
from .utils.recommender import recommend_assets
...
def chat_with_gemini(request):
    ...
    risk_level = request.session.get('risk_level', 'Medium')
    recommendations = request.session.get('last_recommendations', [])

    if not user_message:
        return Response({"error": "No message provided"}, status=400)

    reply = ask_gemini(user_message, context, recommendations=recommendations, risk_level=risk_level)
...
def risk_score(request):
    ...
    recommendations = recommend_assets(risk_level)
